...
import com.lapakkreatiflamongan.smdsforce.schema.Data_MTDReport;


public class Adapter_MTDReport extends BaseAdapter {

    private Context appContext;
    private List<Data_MTDReport> dataValueList;
    ...
    public Adapter_MTDReport (Context appContext, List<Data_MTDReport> dataValueList,float workDayGlobal) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        MTDReportViewHolder viewHolder;
        if (convertView == null) {
            viewHolder = new MTDReportViewHolder();
            ...
            viewHolder = (MTDReportViewHolder) convertView.getTag();
        }

        int procent = 0;
        int procentTimeGone = 0;
        if (Float.parseFloat(dataValueList.get(position).getTargetsales()) > 0) {
            procent = (int)((Float.parseFloat(dataValueList.get(position).getPorcent_rankspv_mtd())));
        }

        if (Float.parseFloat(dataValueList.get(position).getActsales()) > 0) {
            procentTimeGone = ((int) ((procent * 100) / workDayGlobal));
        }

        String actSales = (String) String.valueOf(dataValueList.get(position).getActsales().toString());
        String targetSales = (String) String.valueOf(dataValueList.get(position).getTargetsales().toString());
        viewHolder.tvActualPercent.setText(procent +"%");
        viewHolder.tvNameMTDReport.setText(dataValueList.get(position).getSellername().toString());
        ...
        //viewHolder.tvChannelMTDReport.setText(dataValueList.get(position).getProcent());
    ...
    public class MTDReportViewHolder {
