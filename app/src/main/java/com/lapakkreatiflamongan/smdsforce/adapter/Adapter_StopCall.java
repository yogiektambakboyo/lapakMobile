 ...
 */
public class Adapter_StopCall  extends BaseAdapter {
    ...
    public Adapter_StopCall(Context context, List<Data_Value_Detail_Numbered> initdatalist) {
        mContext = context;
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        holder.Address.setText(""+initdatalist.get(position).getTarget());
        holder.Channel.setText(initdatalist.get(position).getProcent());

        String clockFloat = initdatalist.get(position).getTarget().substring(initdatalist.get(position).getTarget().length()-5,initdatalist.get(position).getTarget().length());

        holder.Procent.setText(""+clockFloat);


        if (((Float.parseFloat(clockFloat.replace(":","")))<1500f)){
            Picasso.get().load(R.drawable.circle_red).into(holder.Img);
            holder.Procent.setTextColor(mContext.getColor(R.color.colorWhite));
        }else if ((((Float.parseFloat(clockFloat.replace(":","")))>=1500f)&&((Float.parseFloat(clockFloat.replace(":","")))<1530f))){
