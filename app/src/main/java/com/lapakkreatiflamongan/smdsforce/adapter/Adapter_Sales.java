...
public class Adapter_Sales  extends BaseAdapter {
    ...
    public Adapter_Sales(Context context, List<Data_Value_Detail_Numbered> initdatalist) {
        mContext = context;
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        holder.Address.setText("Rp. "+formatter.format(Float.parseFloat(initdatalist.get(position).getActual()))+"/ Rp. "+formatter.format(Float.parseFloat(initdatalist.get(position).getTarget())));
        holder.Channel.setText(initdatalist.get(position).getProcent());
        holder.Procent.setText(initdatalist.get(position).getProcent()+"%");


        if (((Float.parseFloat(initdatalist.get(position).getProcent()))>=0f)&&((Float.parseFloat(initdatalist.get(position).getProcent()))<50f)){
